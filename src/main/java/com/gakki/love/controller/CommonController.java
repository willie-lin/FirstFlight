...
import com.gakki.love.domain.*;
import com.gakki.love.domain.dto.CommonResponse;
import com.gakki.love.domain.map.UserRoleMap;
...
public class CommonController {
    ...
    //发表分享时的tags 数
    @Value("${flight.topic.publish.tags.size}")
    private int TAGS_PUBLISH_SIZE;

    //分享页面大小
    @Value("${flight.topic.page.size}")
    private int TOPIC_PAGE_SIZE;
    //达人页面大小
    @Value("${flight.topPeople.page.size}")
    private int PEOPLE_PAGE_SIZE;
    //是否支持滚动页面
    @Value("${flight.page.scroll}")
...
//        清楚访问临时用户信息
    ...
    public String homeTopicHot(
        ...
        try {
            ...
            throw new FlightException("非法参数");
        }
        //获取分页分享
    ...
    public String homeTopPeople(@RequestParam(value = "pageNum", required = false, defaultValue = "1") String pageNum,
        ...
        try {
            ...
            throw new FlightException(WRONG_FORMAT);
        }
        //    获取达人的分页信息
    ...
    public CommonResponse createUser(@Valid User user, BindingResult result) {
        ...
        Roles userRole = rolesServices.getByName(UserRoleMap.USER);
    ...
    public CommonResponse validateUserName(User user, @RequestParam("username") String username) {
        ...
        //    id存在,用户更新
        if (IntegerUtils.isPositiveValue(user.getId())) {
    ...
    public CommonResponse validateEmail(@RequestParam("email") String emailString) throws IOException {
        ...
        if (userService.getByEmail(EncryptUtils.execEncrypt(emailString)) == null) {
